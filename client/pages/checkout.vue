...
async function checkout() {
  if (!selectedAddressId.value) {
    toast.add({ title: 'Vui lòng chọn địa chỉ giao hàng', color: 'yellow' })
    return
  }
  loading.value = true
  try {
    ...
    await cart.reset()
    toast.add({ title: result.message || 'Đặt hàng thành công!', color: 'green' })
    navigateTo(`/orders/${result.orderId}`)
  } catch (e: any) {
    toast.add({ title: e?.data?.message || 'Đặt hàng thất bại', color: 'red' })
  } finally {
    loading.value = false
  }
}
    ...
    <div class="mt-8 grid gap-8 lg:grid-cols-3">
      <div class="lg:col-span-2 space-y-6">
        ...
        <section class="rounded-2xl border border-green-100 bg-white p-6">
          <div class="flex items-center justify-between">
            ...
            <UButton color="green" variant="soft" size="sm" icon="i-heroicons-plus" label="Thêm mới" @click="showAddressForm = !showAddressForm" />
          </div>
          ...
          <form v-if="showAddressForm" class="mt-4 grid gap-3 sm:grid-cols-2" @submit.prevent="createAddress">
            ...
            <div class="sm:col-span-2 flex justify-end gap-2">
              <UButton color="gray" variant="ghost" label="Hủy" @click="showAddressForm = false" />
              <UButton type="submit" color="green" label="Lưu địa chỉ" />
            </div>
          </form>
        ...
        <section class="rounded-2xl border border-green-100 bg-white p-6">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            <label class="flex items-center gap-3 rounded-xl border p-4 transition" :class="paymentMethod === 'COD' ? 'border-green-500 bg-green-50' : 'border-gray-200'" >
              ...
              <span class="grid h-9 w-9 place-items-center rounded-lg bg-green-100 text-green-700"><UIcon name="i-heroicons-banknotes" class="h-5 w-5" /></span>
              <div>
            ...
            <label class="flex items-center gap-3 rounded-xl border p-4 transition" :class="paymentMethod === 'PAYOS' ? 'border-green-500 bg-green-50' : 'border-gray-200'" >
              ...
              <span class="grid h-9 w-9 place-items-center rounded-lg bg-green-100 text-green-700"><UIcon name="i-heroicons-qr-code" class="h-5 w-5" /></span>
              <div>
      ...
      <div class="h-fit rounded-2xl border border-green-100 bg-white p-6 lg:sticky lg:top-20" v-if="cart.cart">
        ...
        <div class="mt-4 space-y-2 border-t border-green-50 pt-4 text-sm">
          ...
          <div class="flex justify-between text-green-600"><span>CO₂ tiết kiệm</span><span>{{ cart.cart.totalCo2Saved.toFixed(2) }} kg</span></div>
        </div>
        ...
        <UButton color="green" size="lg" block class="mt-4" :loading="loading" label="Đặt hàng" icon="i-heroicons-check-circle" @click="checkout" />
      </div>
    </div>
  </div>
</template>