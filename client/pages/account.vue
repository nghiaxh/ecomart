...
async function saveProfile() {
  ...
  try {
    ...
    toast.add({ title: 'Cập nhật thành công', color: 'green' })
  } catch (e: any) {
    toast.add({ title: e?.data?.message || 'Cập nhật thất bại', color: 'red' })
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
    <h1 class="text-3xl font-extrabold text-gray-800">Tài khoản của tôi</h1>
      ...
      <div class="lg:col-span-2">
        <div class="rounded-2xl border border-green-100 bg-white p-6">
          <div class="flex items-center justify-between">
            ...
            <UButton v-if="!editMode" color="green" variant="soft" size="sm" label="Chỉnh sửa" icon="i-heroicons-pencil" @click="startEdit" />
          </div>
          ...
          <form v-if="editMode" class="mt-4 space-y-3" @submit.prevent="saveProfile">
            ...
            <div class="flex justify-end gap-2 pt-2">
              <UButton color="gray" variant="ghost" label="Hủy" @click="editMode = false" />
          ...
          <div v-else-if="profile" class="mt-4 space-y-4">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
              <div class="rounded-xl bg-green-50 p-4">
                ...
                <p class="mt-1 text-xl font-bold text-green-700">{{ profile.ecoPoints ?? 0 }}</p>
              </div>
              <div class="rounded-xl bg-green-50 p-4">
                <p class="text-xs text-gray-500">CO₂ tiết kiệm</p>
                <p class="mt-1 text-xl font-bold text-green-700">{{ (profile.totalCo2Saved ?? 0).toFixed(2) }} kg</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-green-100 bg-white p-6">
          ...
          <div class="mt-3 space-y-2">
            <NuxtLink to="/orders" class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-green-50 hover:text-green-700">
              <UIcon name="i-heroicons-receipt-percent" class="h-4 w-4" /> Đơn hàng
            </NuxtLink>
            <NuxtLink to="/wallet" class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-green-50 hover:text-green-700">
              <UIcon name="i-heroicons-wallet" class="h-4 w-4" /> Ví Eco
            </NuxtLink>
            <NuxtLink to="/chat" class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-gray-600 hover:bg-green-50 hover:text-green-700">
              <UIcon name="i-heroicons-chat-bubble-left-right" class="h-4 w-4" /> Chat hỗ trợ
            </NuxtLink>
          </div>
        </div>
