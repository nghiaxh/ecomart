...
async function addToCart() {
  ...
  try {
    ...
    toast.add({ title: 'Đã thêm vào giỏ hàng', icon: 'i-heroicons-check-circle', color: 'green' })
  } finally {
    adding.value = false
  }
}
...
async function submitReview() {
  ...
  toast.add({ title: 'Cảm ơn bạn đã đánh giá!', icon: 'i-heroicons-check-circle', color: 'green' })
  reviewForm.content = ''
  await loadReviews()
}
...
<template>
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6" v-if="product">
    ...
    <nav class="mb-6 text-sm text-gray-400">
      <NuxtLink to="/" class="hover:text-green-700">Trang chủ</NuxtLink>
      <span class="mx-2">/</span>
      <NuxtLink :to="`/products?category=${product.categorySlug}`" class="hover:text-green-700">{{ product.categoryName }}</NuxtLink>
      <span class="mx-2">/</span>
    ...
    <div class="grid gap-8 lg:grid-cols-2">
      <!-- Gallery -->
      <div>
        <div class="aspect-square overflow-hidden rounded-2xl bg-green-50">
        ...
        <div v-if="product.images && product.images.length > 1" class="mt-3 flex gap-2">
          <button
            ...
            :class="selectedImage === img ? 'border-green-600' : 'border-transparent'"
      ...
      <div>
        <span class="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700">
          <UIcon name="i-heroicons-leaf" class="h-3.5 w-3.5" />
        ...
        <p class="mt-4 text-3xl font-extrabold text-green-700">{{ formatVND(product.price) }}</p>

        <!-- Eco impact -->
        <div class="mt-6 grid grid-cols-2 gap-3">
          <div class="rounded-xl bg-green-50 p-4">
            <p class="flex items-center gap-1 text-xs text-gray-500"><UIcon name="i-heroicons-cloud" class="h-4 w-4 text-green-600" /> CO₂ tiết kiệm</p>
            <p class="mt-1 text-lg font-bold text-green-700">{{ product.co2Saved > 0 ? `-${product.co2Saved.toFixed(1)} kg` : '0 kg' }}</p>
          </div>
          <div class="rounded-xl bg-green-50 p-4">
            <p class="flex items-center gap-1 text-xs text-gray-500"><UIcon name="i-heroicons-star" class="h-4 w-4 text-green-600" /> Điểm Eco</p>
            <p class="mt-1 text-lg font-bold text-green-700">+{{ product.ecoPointsPerUnit }} điểm</p>
          </div>
        </div>
        ...
        <div v-if="product.materials && product.materials.length" class="mt-6">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            <span v-for="m in product.materials" :key="m.id" class="rounded-full border border-green-200 bg-white px-3 py-1 text-xs text-gray-600">
              {{ m.name }} · {{ m.percentage }}%
            </span>
          </div>
        </div>
        ...
        <div class="mt-8 flex flex-wrap items-center gap-4">
          <div class="flex items-center rounded-xl border border-green-200">
            <UButton color="gray" variant="ghost" icon="i-heroicons-minus" :disabled="quantity <= 1" @click="quantity--" />
            <span class="w-10 text-center font-semibold">{{ quantity }}</span>
            <UButton color="gray" variant="ghost" icon="i-heroicons-plus" :disabled="quantity >= product.stock" @click="quantity++" />
          </div>
          <UButton
            color="green"
            size="lg"
    ...
    <!-- Reviews -->
    <div class="mt-12 border-t border-green-100 pt-8">
      <h2 class="text-2xl font-extrabold text-gray-800">Đánh giá</h2>

      <div v-if="isLoggedIn" class="mt-6 rounded-2xl border border-green-100 bg-white p-4">
        ...
        <div class="mt-2 flex items-center gap-1">
          <button v-for="s in 5" :key="s" :class="s <= reviewForm.rating ? 'text-yellow-400' : 'text-gray-300'" @click="reviewForm.rating = s">
            <UIcon name="i-heroicons-star-solid" class="h-6 w-6" />
          </button>
        </div>
        <div class="mt-3 flex items-end gap-3">
          <UTextarea v-model="reviewForm.content" placeholder="Chia sẻ trải nghiệm của bạn..." class="flex-1" :rows="2" />
          <UButton color="green" label="Gửi" icon="i-heroicons-paper-airplane" @click="submitReview" />
        </div>
      </div>

      <div class="mt-6 space-y-4">
        <div v-for="r in reviews" :key="r.id" class="rounded-2xl border border-green-50 bg-white p-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 text-xs text-gray-400">
              <span class="flex gap-0.5">
                <UIcon v-for="s in 5" :key="s" :name="s <= r.rating ? 'i-heroicons-star-solid' : 'i-heroicons-star'" :class="s <= r.rating ? 'text-yellow-400' : 'text-gray-300'" class="h-4 w-4" />
              </span>
