...
async function updateStatus(o: Order, status: string) {
  ...
  toast.add({ title: 'Đã cập nhật trạng thái', color: 'green' })
  await load()
}

async function confirmPayment(o: Order) {
  await request(`/api/orders/${o.id}/confirm-payment`, { method: 'POST' })
  toast.add({ title: 'Đã xác nhận thanh toán', color: 'green' })
  await load()
}
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <div class="flex flex-col items-end gap-2">
            ...
            <div class="flex gap-2">
              ...
              />
              <UButton v-if="o.payment.status === 'PENDING'" color="green" variant="soft" size="sm" label="Xác nhận thanh toán" @click="confirmPayment(o)" />
            </div>
          </div>
        </div>
      </div>
