...
const form = reactive({
  ...
  carbonIndex: 0, baselineCarbonIndex: 0, ecoPointsPerUnit: 0, weight: 0,
  origin: '', categoryId: 0, active: true, imageUrl: ''
})
...
function openCreate() {
  ...
  Object.assign(form, { name: '', slug: '', description: '', price: 0, stock: 0, carbonIndex: 0, baselineCarbonIndex: 0, ecoPointsPerUnit: 0, weight: 0, origin: '', categoryId: 0, active: true, imageUrl: '' })
  errors.value = {}
  showForm.value = true
}
...
function openEdit(p: Product) {
  ...
  Object.assign(form, {
    ...
    carbonIndex: p.carbonIndex, baselineCarbonIndex: p.baselineCarbonIndex, ecoPointsPerUnit: p.ecoPointsPerUnit,
...
async function submit() {
  ...
  const payload: ProductRequest = {
    ...
    carbonIndex: Number(form.carbonIndex || 0), baselineCarbonIndex: Number(form.baselineCarbonIndex || 0),
    ecoPointsPerUnit: Number(form.ecoPointsPerUnit || 0), weight: Number(form.weight || 0), origin: form.origin,
  ...
  try {
    ...
    toast.add({ title: 'Đã lưu sản phẩm', color: 'green' })
...
async function remove(p: Product) {
  ...
  toast.add({ title: 'Đã xóa sản phẩm', color: 'green' })
  await load()
}
...
<template>
  <div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      ...
      <UButton color="green" icon="i-heroicons-plus" label="Thêm sản phẩm" @click="openCreate" />
    </div>
    ...
    <div v-if="showForm" class="mb-6 rounded-2xl border border-green-100 bg-white p-6">
      ...
      <form class="grid gap-4 md:grid-cols-3" @submit.prevent="submit">
        ...
        <div class="md:col-span-1">
          <label class="mb-1 block text-sm text-gray-500">Điểm Eco</label>
          <UInput v-model="form.ecoPointsPerUnit" type="number" step="0.1" />
        </div>
        <div class="md:col-span-1">
          <label class="mb-1 block text-sm text-gray-500">Chỉ số CO₂</label>
          <UInput v-model="form.carbonIndex" type="number" step="0.1" />
        </div>
        <div class="md:col-span-1">
          <label class="mb-1 block text-sm text-gray-500">CO₂ gốc</label>
          <UInput v-model="form.baselineCarbonIndex" type="number" step="0.1" />
        </div>
        <div class="md:col-span-1">
    ...
    <div class="mb-4 flex items-center gap-3">
      <UInput v-model="search" icon="i-heroicons-magnifying-glass" placeholder="Tìm sản phẩm..." class="max-w-md" />
    </div>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id" class="border-t border-gray-100">
            ...
            <td class="px-4 py-3">
              <UBadge :color="p.active ? 'success' : 'gray'" :label="p.active ? 'Bán' : 'Ẩn'" size="sm" />
            </td>
            <td class="px-4 py-3">
              <div class="flex justify-end gap-1">
                <UButton color="gray" variant="ghost" icon="i-heroicons-pencil-square" @click="openEdit(p)" />
                <UButton color="gray" variant="ghost" :icon="p.active ? 'i-heroicons-eye-slash' : 'i-heroicons-eye'" @click="toggle(p)" />
                <UButton color="gray" variant="ghost" icon="i-heroicons-trash" @click="remove(p)" />
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="totalPages > 1" class="mt-4 flex items-center justify-center gap-2">
      <UButton color="gray" variant="soft" icon="i-heroicons-chevron-left" :disabled="page === 0" @click="page--; load()" />
      <span class="px-3 text-sm">Trang {{ page + 1 }} / {{ totalPages }}</span>
      <UButton color="gray" variant="soft" icon="i-heroicons-chevron-right" :disabled="page >= totalPages - 1" @click="page++; load()" />
    </div>
  </div>
</template>