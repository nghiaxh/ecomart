...
const cards = computed(() => [
  { label: 'Sản phẩm', value: stats.value?.productCount ?? 0, icon: 'i-heroicons-shopping-bag', color: 'text-blue-600' },
  { label: 'Khách hàng', value: stats.value?.customerCount ?? 0, icon: 'i-heroicons-users', color: 'text-green-600' },
  { label: 'Đơn hàng', value: stats.value?.orderCount ?? 0, icon: 'i-heroicons-receipt-percent', color: 'text-orange-600' },
  { label: 'Doanh thu', value: stats.value ? formatVND(stats.value.revenue) : '0', icon: 'i-heroicons-banknotes', color: 'text-purple-600' }
])
</script>
    ...
    <div class="mt-8 grid gap-4 lg:grid-cols-2">
      <NuxtLink to="/admin/orders" class="rounded-2xl border border-green-100 bg-gradient-to-br from-green-50 to-white p-6 transition hover:shadow-md">
        <div class="flex items-center justify-between">
          ...
          <span class="grid h-12 w-12 place-items-center rounded-2xl bg-green-600 text-white"><UIcon name="i-heroicons-arrow-right" class="h-6 w-6" /></span>
        </div>
      </NuxtLink>
