...
async function submit() {
  ...
  try {
    ...
    toast.add({ title: 'Đã lưu banner', color: 'green' })
...
async function remove(b: Banner) {
  ...
  toast.add({ title: 'Đã xóa banner', color: 'green' })
  await load()
}
...
<template>
  <div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
      ...
      <UButton color="green" icon="i-heroicons-plus" label="Thêm banner" @click="openCreate" />
    </div>
      ...
      <div v-for="b in banners" :key="b.id" class="overflow-hidden rounded-2xl border border-gray-200 bg-white">
        ...
        <div class="p-4">
          <div class="flex items-center justify-between">
            ...
            <UBadge :color="b.active ? 'success' : 'gray'" :label="b.active ? 'Hiển thị' : 'Ẩn'" size="sm" />
          </div>
          <p v-if="b.subtitle" class="mt-1 text-sm text-gray-400">{{ b.subtitle }}</p>
          <div class="mt-3 flex justify-end gap-1">
            <UButton color="gray" variant="ghost" icon="i-heroicons-pencil-square" @click="openEdit(b)" />
            <UButton color="gray" variant="ghost" icon="i-heroicons-trash" @click="remove(b)" />
          </div>
        </div>
      </div>
