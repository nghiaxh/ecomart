...
- **DB schema**: `ddl-auto: update` by default (`JPA_DDL_AUTO` overrides) keeps the schema in sync. Flyway is enabled (`baseline-on-migrate: true`, `baseline-version: 1`, `locations: classpath:db/migration`) and already holds one real migration: `V2__drop_banners.sql`. Prefer adding new Flyway scripts (e.g. `V3__...`) over relying on `ddl-auto` when you can.
