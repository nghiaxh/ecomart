...
- **client/** — Vue 3 + Vite 8 + Vue Router 5 + Axios + TS + Zod 4 + Tailwind CSS 4 (via `@tailwindcss/vite`), with **PrimeVue 4.5** (Aura preset, theme from `@primevue/themes`, MIT license) + **PrimeIcons**. SPA only, no SSR. App code in `src/` (`main.ts` + `App.vue` + `router/` + `layouts/` + `pages/` + `components/`, `composables/`, `types/`, `schemas/`, `data/`, `assets/`). **No auto-imports, no unplugin** — Vue core, `vue-router`, `@vueuse/core`, every PrimeVue component, and local components/composables are imported explicitly per file. Native PrimeVue `useToast` (`primevue/usetoast`) is used for toasts, `useConfirm` (`primevue/useconfirm`) for confirms, and `useHead` from `@unhead/vue` for head; these explicit imports let unit tests `vi.mock('primevue/usetoast')`. `App.vue` mounts `<Toast position="top-center" />` + `<ConfirmDialog />`; `main.ts` installs `ToastService` + `ConfirmationService` and `app.use(PrimeVue, { theme: { preset: Aura } })`. Icons are `<i class="pi pi-*" />` (PrimeIcons). UI text and Zod error messages are Vietnamese. Shared UI in `src/components/` (`UiImg`, `PaginationBar`, `OrderSummaryCard`, `AddressForm/Card`, `ProductCard`, `AddToCartButton`, `PasswordInput`, `AuthShell`, `FooterGlobal`, `Reveal`, `SectionHeader`); form errors via `useFormErrors`; status→`Tag` severity mapping in `useStatusLabels` (`badgeSeverity`); session keys owned by `src/utils/session-storage.ts`; static home content in `src/data/home.ts`.
