...
- **DB schema**: `ddl-auto: update` by default (`JPA_DDL_AUTO` overrides) keeps the schema in sync. Flyway is enabled (`baseline-on-migrate: true`, `baseline-version: 1`, `locations: classpath:db/migration`); the migration folder currently holds no real scripts (only `.gitkeep`). Prefer adding new Flyway scripts (e.g. `V2__...`) over relying on `ddl-auto` when you can.
...
- **No chat and no client notification UI**: the chat feature (ChatService/ChatSession/ChatMessage) has been removed; the server exposes `/api/notifications` but no page polls it yet. Don't reintroduce chat, don't document it as a live feature.
